            ...
            if (!empty($res)) {
                if ($this->ion_auth_model->in_group('admin', $res[0]['id'])) {
                    ...
                    if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember, 'phone')) {
                        ...
                        if (!$this->input->is_ajax_request() && ($seller_status == '1')) {
                    ...
                    if ($affiliate_status == '0' || $affiliate_status == '2') {
                        ...
                        $response['csrfName'] = $this->security->get_csrf_token_name();
                        $response['csrfHash'] = $this->security->get_csrf_hash();
                        $response['message'] = $messages[$affiliate_status];
                        echo json_encode($response);
                        return;
                    }
                    // check for "remember me"
                    ...
                    if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember, 'phone')) {
                        //if the login is successful
                        $affiliate_status = $this->ion_auth->affiliate_status($this->session->userdata('user_id'));
