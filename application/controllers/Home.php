...
class Home extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $brands = $this->brand_model->get_brands('', $limit, $offset, $sort, $order, '1');
        ...
        if (!empty($sections)) {
            for ($i = 0; $i < count($sections); $i++) {
                ...
                if (isset($_SESSION['theme']) && !empty($_SESSION['theme'])) {
                    $theme = fetch_details('themes', ['name' => $_SESSION['theme']]);
                } else {
    ...
    public function brands()
    {
        $sort = 'row_order';
        $order =  'ASC';
        $limit = ($this->input->get('per-page')) ? $this->input->get('per-page', true) : 18;
        $brands = $this->brand_model->get_brands('', '', '', $sort, $order, '1');
        ...
        $config['full_tag_open'] = '<ul class="pagination justify-content-center px-10">';
        ...
        $this->data['brands'] = $this->brand_model->get_brands('', $limit, $offset, $sort, $order, '1');;
    ...
    public function get_products()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            $data = suggestionKeyword($_GET['search']);
            ...
            if (!empty($products['product'])) {
                ...
                $this->response['suggestion_keywords'] = $data;
    ...
    public function verifyUser()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            if (!empty($user_data)) {
                $username = $this->session->set_userdata('username', $user_data[0]['username']);
                $user_id = $this->session->set_userdata('user_id', $user_data[0]['id']);
                $this->response['error'] = false;
