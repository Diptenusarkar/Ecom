...
class Products extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $brands = get_assigned_brands();
        // echo "<pre>";
        // print_r($brands);

        // $category = fetch_details('categories', ['status' => 1]);
        $category = get_assigned_categories();
    ...
    public function details($slug = '')
    {
        ...
        $this->data['reference_id'] = (isset($_GET['ref']) && !empty($_GET['ref'])) ? $_GET['ref'] : '';
        $this->data['payment_methods'] = $this->data['payment_method'];
    ...
    public function search()
    {
        ...
        for ($i = 0; $i < count($products['product']); $i++) {
            ...
            $product = array(
                ...
                'image' => $products['product'][$i]['image'],
                'url' => base_url('products/details/' . $products['product'][$i]['slug']),
                'description' => output_escaping(
                    str_replace(
                        '\r\n',
                        '&#13;&#10;',
                        isset($products['product'][$i]['short_description']) && !empty($products['product'][$i]['short_description'])
                            ? $products['product'][$i]['short_description']
                ...
                'offers' => array(
                    '@type' => 'AggregateOffer',
                    'priceCurrency' => $this->data['settings']['supported_locals'],
                    'highPrice' => $products['product'][$i]['min_max_price']['max_price'],
                    'lowPrice' => $products['product'][$i]['min_max_price']['max_special_price'],
                ),

            );

            // Add rating if available
            if ($products['product'][$i]['rating'] > 0 && $products['product'][$i]['no_of_ratings'] > 0) {
                $product['aggregateRating'] = array(
                    '@type' => 'AggregateRating',
                    'ratingValue' => $products['product'][$i]['rating'],
                    'reviewCount' => $products['product'][$i]['no_of_ratings'],
                );
            }
            // Add product to itemListElement array
            $product_list['itemListElement'][] = $product;
        ...
        // Pass product details and JSON-LD data to the view
        $this->data['data_json_ld'] = isset($product_list) && !empty($product_list) ? json_encode($product_list, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) : '';
