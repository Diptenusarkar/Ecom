...
class Affiliate_policies extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            $this->data['main_page'] = AFFILIATE . 'privacy-policy';
            $settings = get_settings('system_settings', true);
            $this->data['title'] = 'Affiliate Privacy Policy | ' . $settings['app_name'];
            $this->data['meta_description'] = 'Affiliate Privacy Policy | ' . $settings['app_name'];
            $this->data['privacy_policy'] = get_settings('affiliate_privacy_policy');
            $this->data['terms_n_condition'] = get_settings('affiliate_terms_conditions');
    ...
    public function update_privacy_policy_settings()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                $this->Setting_model->update_affiliate_privacy_policy($privacy_policy_input_description);
                $this->Setting_model->update_affiliate_terms_n_condtions($terms_n_conditions_input_description);
    ...
    public function privacy_policy_page()
    {
        ...
        $this->data['privacy_policy'] = get_settings('affiliate_privacy_policy');
        $this->load->view('admin/pages/affiliate/view-privacy-policy', $this->data);
    }
    ...
    public function terms_and_conditions_page()
    {
        ...
        $this->data['terms_and_conditions'] = get_settings('affiliate_terms_conditions');
        $this->load->view('admin/pages/affiliate/terms-and-conditions', $this->data);
    }
}
