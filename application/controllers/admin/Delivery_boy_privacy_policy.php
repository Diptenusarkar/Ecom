...
class Delivery_boy_privacy_policy extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            $this->data['main_page'] = FORMS . 'delivery-boy-privacy-policy';
            $settings = get_settings('system_settings', true);
            $this->data['title'] = 'Delivery Boy Privacy Policy | ' . $settings['app_name'];
            $this->data['meta_description'] = 'Delivery Boy Privacy Policy | ' . $settings['app_name'];
    ...
    public function update_privacy_policy_settings()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                $this->Setting_model->update_delivery_boy_privacy_policy($privacy_policy_input_description);
                $this->Setting_model->update_delivery_boy_terms_n_condtions($terms_n_conditions_input_description);
    ...
    public function terms_and_conditions_page()
    {
        ...
        $this->data['terms_and_conditions'] = get_settings('delivery_boy_terms_conditions');
