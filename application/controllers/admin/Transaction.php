...
class Transaction extends CI_Controller {
	...
	public function view_transactions()
	{
		if($this->ion_auth->logged_in() && $this->ion_auth->is_admin())
		{			
			return $this->Transaction_model->get_transactions_list(group_id: 1);
		}
		else{
			redirect('admin/login','refresh');
		}
    ...
    public function edit_transactions()
    {
		...
		{			
            ...
            if (!$this->form_validation->run()) {
                ...
                $message = (isset($_POST['message']) && trim($_POST['message']) != "") ? $this->input->post('message', true) : "";
                $this->Transaction_model->edit_transactions($message);
