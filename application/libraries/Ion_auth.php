...
class Ion_auth
{
	...
	public function register($identity, $password, $email, $additional_data = [], $group_ids = [])
	{
		...
		$email_check = fetch_details("welcome_wallet_users", ["email" => $email]);
		$mobile_check = fetch_details("welcome_wallet_users", ["mobile" => $identity]);

		if (count($email_check) == 0) {
			insert_details(["email" => $email, "type" => "google", "created_at" => date("Y-m-d H:i:s")], "welcome_wallet_users");
		}
		if (count($mobile_check) == 0) {
			insert_details(["mobile" => $identity, "type" => "google", "created_at" => date("Y-m-d H:i:s")], "welcome_wallet_users");
		}
	...
	public function is_seller($id = FALSE)
	{
	...
	public function affiliate_status($id = FALSE)
	{
		$id = ($id == FALSE) ? $this->session->userdata('user_id') : $id;
		if (!empty($id) && $id != FALSE) {
			$status = fetch_details('affiliates', ['user_id' => $id], 'status');
			return $status[0]['status'];
		} else {
			return false;
		}
		$affiliate_group = $this->config->item('affiliate_group', 'ion_auth');

		return $this->ion_auth_model->in_group($affiliate_group, $id);
	}
	...
	public function is_affiliate_user($id = FALSE)
	{
