...
class Notification_model extends CI_Model
{
    ...
    function get_notifications($offset, $limit, $sort, $order, $user_id = null)
    {
        ...
        if ($user_id !== null) {
            $this->db->group_start();
            // For notifications sent to all users
            $this->db->where('send_to', 'all_users');
            // For notifications sent to specific user
            $this->db->or_like('users_id', '"' . $user_id . '"');
            $this->db->group_end();
        }
        $count_res = $this->db->select(' COUNT(id) as total ')->get()->result_array();
        ...
        $search_res = $this->db->select(' * ')->order_by($sort, $order)->limit($limit, $offset)->get()->result_array();
    ...
    public function get_notifications_data($offset = 0, $limit = 10, $sort = 'read_by', $order = 'DESC')
    {
        ...
        foreach ($city_search_res as $row) {
            ...
            if (isset($row['type']) && $row['type'] == 'return_request') {
                $operate .= '<a href=' . base_url('admin/return-request') . '?edit_id=' . $row['type_id'] . '&noti_id=' . $row['id'] . ' class="btn action-btn btn-primary btn-xs ml-1 mr-1 mb-1" title="View Return Request" ><i class="fa fa-eye"></i></a>';
            } else {
