...
$(document).ready(function () {
    ...
    $(document).on("click", '.reorder-btn', (event) => {
...
$(function () {
                ...
                if (selected_attributes.length == attributes_length) {
                    ...
                    if (is_variant_available) {
                        ...
                        if (prices[0].special_price < prices[0].price && prices[0].special_price != 0) {
                            ...
                            //  $('#modal-add-to-cart-button').attr('data-product-special-price', price);
    ...
    $('#modal-add-to-cart-button').on('click', function (e) {
        ...
        var stock = parseFloat($(this).attr('data-product-low-stock-limit'));
        var effectiveLowStockLimit = getEffectiveLowStockLimit(stock);
        ...
        if (isNaN(stock) && stock <= effectiveLowStockLimit) {
            Toast.fire({
                icon: "error",
    ...
    if (cart !== null && cart.length > 0) {
        ...
        cart.forEach((item) => {
            ...
            html += '<div class="row">' +
                ...
                '<div class="product-price align-self-center">' + currency + ' ' + item.special_price + '</div>' +
                ...
                '</button>' +
                '</div>' +
                '<div class="product-line-price align-self-center px-1">' + currency + ' ' + (item.qty * item.special_price).toLocaleString(undefined, { minimumFractionDigits: 2 }) + '</div>' +
...
$('#city, #city_modal').on('change', function (e) {
    e.preventDefault();
    var value = $(this).val()
    console.log(value);
